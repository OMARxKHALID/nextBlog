  ...
  return (
    <nav>
      <div className="block xl:hidden">
        <div
          className={`overflow-y-auto z-40 flex pt-5 top-0 flex-col h-screen w-full max-w-[250px] fixed bg-base-200  duration-500 ease-in  gap-2 md:gap-0 shadow-xl ${
            sidebarOpen ? "left-0" : "-left-full"
          }`}
        >
          <div className="relative flex flex-col gap-5 px-5 pb-6 mt-4 text-lg font-normal leading-6">
            <div className="flex items-center justify-between mb-3">
              <h4 className="text-base font-semibold text-base-content/80">
                Menu
              </h4>
              <svg
            ...
            {/*  search box */}
            <div className="bg-base-200 px-2 py-2 rounded-md flex items-center gap-3 border sm:hidden">
              <input
                type="text"
                className="outline-none text-base-content bg-base-200 w-40 placeholder:font-work"
                placeholder="Search"
              />
              <svg
                width="16"
            ...
            {headerData.map((item, index) => (
              <Link
                href={item.link}
                key={index}
                className="link link-hover text-base text-base-content/80 hover:text-primary transition hover:duration-300 font-work font-semibold "
              >
                {item.name}
              </Link>
            ))}
